...
// ============================================================
//  REAL-TIME TELEMETRY ENGINE & ECG MONITOR
// ============================================================
let telemetryAudioCtx = null;
let telemetryAudioMuted = true;
let telemetryECGPoints = [];
let telemetryX = 0;
let telemetryTime = 0;

function initTelemetryEngine() {
  const canvas = document.getElementById('telemetryEcgCanvas');
  if (!canvas) return;
  const ctx = canvas.getContext('2d');

  const audioBtn = document.getElementById('telemetryAudioToggle');
  const audioIcon = document.getElementById('telemetryAudioIcon');

  if (audioBtn) {
    audioBtn.addEventListener('click', () => {
      if (!telemetryAudioCtx) {
        telemetryAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
      }
      telemetryAudioMuted = !telemetryAudioMuted;
      audioBtn.classList.toggle('active', !telemetryAudioMuted);
      audioIcon.textContent = telemetryAudioMuted ? '🔇' : '🔊';
    });
  }

  function playHeartSound() {
    if (telemetryAudioMuted || !telemetryAudioCtx) return;
    try {
      // LUB (AV Valve closure)
      const osc1 = telemetryAudioCtx.createOscillator();
      const gain1 = telemetryAudioCtx.createGain();
      osc1.type = 'sine';
      osc1.frequency.setValueAtTime(65, telemetryAudioCtx.currentTime);
      osc1.frequency.exponentialRampToValueAtTime(30, telemetryAudioCtx.currentTime + 0.12);
      gain1.gain.setValueAtTime(0.5, telemetryAudioCtx.currentTime);
      gain1.gain.exponentialRampToValueAtTime(0.001, telemetryAudioCtx.currentTime + 0.12);
      osc1.connect(gain1);
      gain1.connect(telemetryAudioCtx.destination);
      osc1.start();
      osc1.stop(telemetryAudioCtx.currentTime + 0.12);

      // DUB (Semilunar Valve closure)
      setTimeout(() => {
        if (telemetryAudioMuted || !telemetryAudioCtx) return;
        const osc2 = telemetryAudioCtx.createOscillator();
        const gain2 = telemetryAudioCtx.createGain();
        osc2.type = 'sine';
        osc2.frequency.setValueAtTime(90, telemetryAudioCtx.currentTime);
        osc2.frequency.exponentialRampToValueAtTime(40, telemetryAudioCtx.currentTime + 0.08);
        gain2.gain.setValueAtTime(0.35, telemetryAudioCtx.currentTime);
        gain2.gain.exponentialRampToValueAtTime(0.001, telemetryAudioCtx.currentTime + 0.08);
        osc2.connect(gain2);
        gain2.connect(telemetryAudioCtx.destination);
        osc2.start();
        osc2.stop(telemetryAudioCtx.currentTime + 0.08);
      }, 150);
    } catch(e) {}
  }
  ...
  function getEcgY(t) {
    ...
    if (diseaseMode === 'arrhythmia') {
      // Fibrillating erratic wave
      const noise = (Math.random() - 0.5) * 10;
      const flutter = Math.sin(t * 35) * 6;
      const extraSpike = (cycle > 0.4 && cycle < 0.44) ? Math.sin((cycle - 0.4)/0.04 * Math.PI) * 45 : 0;
      return noise + flutter + extraSpike;
    }

    // Normal or ST-elevation (blocked)
    if (cycle > 0.12 && cycle < 0.22) {
      // P Wave
      y = Math.sin((cycle - 0.12) / 0.10 * Math.PI) * 8;
    } else if (cycle >= 0.26 && cycle < 0.29) {
      // Q Dip
      y = -6;
    } else if (cycle >= 0.29 && cycle <= 0.35) {
      // QRS Peak
      const qrsT = (cycle - 0.29) / 0.06;
      y = Math.sin(qrsT * Math.PI) * 42;
    } else if (cycle > 0.35 && cycle < 0.39) {
      // S Dip
      y = -10;
    } else if (cycle >= 0.39 && cycle <= 0.48 && diseaseMode === 'blocked') {
      // ST Segment Elevation (MI signal)
      y = 12;
    } else if (cycle > 0.50 && cycle < 0.65) {
      // T Wave
      const tMult = (diseaseMode === 'blocked') ? -1 : 1;
      y = Math.sin((cycle - 0.50) / 0.15 * Math.PI) * 12 * tMult;
    }
  ...
  function renderEcg() {
    ...
    const bpm = heartRateBPM || 72;
    const speed = (bpm / 60) * 0.012;
    telemetryTime += speed;

    const currentCycle = Math.floor(telemetryTime);
    if (currentCycle > lastBeatCycle) {
      lastBeatCycle = currentCycle;
      playHeartSound();
    }

    const valY = centerY - getEcgY(telemetryTime);

    telemetryECGPoints.push({ x: telemetryX, y: valY });
    if (telemetryECGPoints.length > width) {
      telemetryECGPoints.shift();
    }
    ...
    // Draw ECG Trace
    ctx.beginPath();
    ctx.strokeStyle = (diseaseMode === 'arrhythmia' || diseaseMode === 'blocked') ? '#ff3355' : '#00ffd5';
    ctx.lineWidth = 2;

    for (let i = 0; i < telemetryECGPoints.length; i++) {
      const pt = telemetryECGPoints[i];
      if (i === 0) ctx.moveTo(i, pt.y);
      else ctx.lineTo(i, pt.y);
    }
    ctx.stroke();

    // Lead Dot
    if (telemetryECGPoints.length > 0) {
      const lastPt = telemetryECGPoints[telemetryECGPoints.length - 1];
      ctx.fillStyle = '#ffffff';
      ctx.shadowColor = '#00ffd5';
      ctx.shadowBlur = 8;
      ctx.beginPath();
      ctx.arc(telemetryECGPoints.length - 1, lastPt.y, 3, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;
    }

    telemetryX = (telemetryX + 1) % width;

    // Update Telemetry Metrics
    updateTelemetryStats(telemetryTime % 1.0);
  }

  function updateTelemetryStats(progress) {
    const bpm = heartRateBPM || 72;
    const teleHR = document.getElementById('teleHR');
    const teleBP = document.getElementById('teleBP');
    const teleCO = document.getElementById('teleCO');
    const teleSpO2 = document.getElementById('teleSpO2');
    const teleCyclePhase = document.getElementById('teleCyclePhase');
    const teleCycleFill = document.getElementById('teleCycleFill');
    const ecgPhaseTag = document.getElementById('ecgPhaseTag');

    if (teleHR) teleHR.textContent = bpm;

    // Blood Pressure
    let sys = Math.round(90 + (bpm / 72) * 30);
    let dia = Math.round(60 + (bpm / 72) * 20);
    if (diseaseMode === 'blocked') { sys += 25; dia += 15; }
    if (teleBP) teleBP.textContent = `${sys}/${dia}`;

    // Cardiac Output
    const sv = diseaseMode === 'valve' ? 45 : 70; // mL
    const co = ((bpm * sv) / 1000).toFixed(1);
    if (teleCO) teleCO.textContent = co;

    // SpO2
    let spo2 = '98%';
    if (diseaseMode === 'blocked') spo2 = '88%';
    else if (diseaseMode === 'valve') spo2 = '91%';
    else if (diseaseMode === 'arrhythmia') spo2 = '93%';
    if (teleSpO2) teleSpO2.textContent = spo2;

    // Cardiac Cycle Phase
    let phaseName = 'Diastole (Filling)';
    if (progress < 0.20) phaseName = 'Atrial Systole';
    else if (progress < 0.45) phaseName = 'Ventricular Systole';
    else if (progress < 0.65) phaseName = 'Isovolumetric Relaxation';

    if (teleCyclePhase) teleCyclePhase.textContent = phaseName;
    if (ecgPhaseTag) ecgPhaseTag.textContent = phaseName.toUpperCase();
    if (teleCycleFill) teleCycleFill.style.width = `${progress * 100}%`;
  }
